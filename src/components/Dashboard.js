    ...
    const fetchData = async () => {
      try {
        ...
        const cleanedData = rows.slice(1).map((row) => {
          ...
          return {
            date: values["Date"],
            requestsSent: parseInt(values["Requests sent"], 10) || 0,
            position: values["Position"] || "",
            acceptedRequests: parseInt(values["Accepted requests"], 10) || 0,
            totalAccepted:
              parseInt(values["Total number of accepted requests"], 10) || 0,
          };
        });
